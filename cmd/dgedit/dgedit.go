...
import (
	...
	"github.com/jan-g/oci-poledit/dg"
...
func main() {
	...
	if flag.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "Specify one path to a dynamic-group object")
	}
	...
	dg, err := dgHandler.Find(context.Background(), *compartment.Id, policyName, iam)
	if err == nil {
		if *new {
			panic(errors.New("dynamic-group already exists"))
		}
		if *description != "" {
			dg = dg.WithDescription(description)
		}
		dg, err = editor(dg)
		if err != nil {
			panic(err)
		...
		if !*new {
			panic(errors.New("dynamic-group does not exist; use -create"))
		}
		if *description != "" {
			dg = dg.WithDescription(description)
		} else {
		...
		if dg.ShouldCreate() {
			err := dg.DoCreate(iam, compartment)
			if err != nil {
				panic(err)
			}
		}
